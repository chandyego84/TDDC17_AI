	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				// ***REFER TO BELLMAN EQUATION*** //
				double temporalDifference = (previous_reward + GAMMA_DISCOUNT_FACTOR * 
						getMaxActionQValue(new_state) - Qtable.get(prev_stateaction));
				
				double qValue = Qtable.get(prev_stateaction) + alpha(Ntable.get(prev_stateaction)) * temporalDifference;
